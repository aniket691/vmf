    ...
    if (token) {
      try {
        ...
        if (userId) {
          setDistributorId(userId);
          // Fetch both together
          fetchCertificatesAndDocuments(userId);
        } else {
          console.error("User ID is missing in the decoded token.");
        }
      } catch (err) {
  ...
  const fetchCertificatesAndDocuments = async (userId) => {
    try {
      const [certRes, docRes] = await Promise.all([
        axios.get("http://localhost:3000/certificates", authHeaders),
        axios.get(`http://localhost:3000/documents/list/${userId}`, authHeaders),
      ]);

      const certs = certRes.data;
      const docs = docRes.data.documents;

      // Filter only documents that are NOT 'Sent'
      const filtered = docs.filter((doc) => doc.status !== "Sent");

      // Filter out those that already have BOTH receipt and certificate
      const finalFiltered = filtered.filter((doc) => {
        const hasReceipt = !!doc.receipt_url;
        const cert = certs.find(
          (c) => String(c.document_id) === String(doc.document_id)
        );
        const hasCertificate = !!cert?.certificate_id;
        return !(hasReceipt && hasCertificate); // only include if at least one is missing
      });

      const sorted = finalFiltered.sort(
      ...
      console.error("Error fetching documents or certificates:", err);
    }
  };
