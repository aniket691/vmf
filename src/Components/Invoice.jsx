...
const InvoicePage = () => {
  ...
  const handleAssignDistributor = async (distributorId) => {
    ...
    try {
      ...
      // ✅ Step 4: Send SMS notification to applicant
      let raw = phone.replace(/^0+/, ""); // strip leading zeros

      const applicantPhone = raw.phone?.startsWith("91")
        ? documentData.phone
        : "91" + documentData.phone;

      const smsMessage =
        `Congratulations! Your application (ID: ${documentData.application_id}) has been *approved*.\n\n` +
        `It is now forwarded to our distributor for further processing.\n\n` +
        `Thank you for choosing Mazedakhale!`;
                  ...
                  <td className="p-1 border border-gray-300 text-center">
                    <input
                      ...
                      onChange={() => {
                        ...
                        handleDocumentClick(doc.file_path, index); // 👈 Open popup when checkbox is clicked
                      }}
                    />
                  </td>
                  <td
