...
const token = localStorage.getItem("token");
  ...
  //     // ✅ SMS logic: only send when status is "Completed" or similar
  //     if (newStatus.toLowerCase() === "received") {
  //       const updatedDoc = response.data; // or fetch full document info if needed
  //       const phone = updatedDoc.phone?.startsWith("91")
  //         ? updatedDoc.phone
  //         : "91" + updatedDoc.phone;
  ...
  //         `Please login to your account and download it from the dashboard.\n\n` +
  //         `Thank you!\n- Mazedakhale Team`;
  ...
  const handleUpdateStatus = async (
    ...
    try {
      ...
      // ✅ SMS logic after status update
      let raw = phone.replace(/^0+/, ""); // strip leading zeros

      const formattedPhone = raw.startsWith("91") ? phone : "91" + phone;
      const formattedName = name;
      const formattedApplicationid = Applicationid;
      const message =
        `Dear ${formattedName},\n\n` +
        `Your ${formattedApplicationid} No receipt is ready to download.` +
        `Please login to your account to get receipt.\n\n` +
        `Thank you!\n- Mazedakhale Team`;
